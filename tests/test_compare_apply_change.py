...
from ksbody.web.routes.compare import (
...
class CompareApplyChangeTests(unittest.TestCase):
    ...
    def test_diff_rows_marks_value_vs_none_as_diff(self) -> None:
        rows = [{"recipe_import_id": 1, "file_type": "PRM", "param_name": "Bond1_Force_Seg_01", "param_value": "150"}]
    ...
    def test_diff_rows_keeps_both_none_as_no_diff(self) -> None:
        rows = [
            {"recipe_import_id": 1, "file_type": "PRM", "param_name": "Bond1_Force_Seg_01", "param_value": None},
            {"recipe_import_id": 2, "file_type": "PRM", "param_name": "Bond1_Force_Seg_01", "param_value": None},
        ]
    ...
    def test_diff_rpm_rows_supports_multi_field_output(self) -> None:
        keys = ["signal_name", "property_name", "rpm_group", "bond_type", "measurement_name", "limit_type", "statistic_type", "parameter_set"]
        rows = [{"recipe_import_id": 1, "signal_name": "S1", "property_name": "P1", "rpm_group": "G1", "bond_type": "BT", "measurement_name": "M", "limit_type": "L", "statistic_type": "ST", "parameter_set": "PS", "lower_limit": 1.0, "upper_limit": 2.0, "active": True}]
        output = _diff_rpm_rows(keys, ["lower_limit", "upper_limit", "active"], rows, [1, 2], show_all=False)
        self.assertEqual({row["field"] for row in output}, {"lower_limit", "upper_limit", "active"})
    ...
    def test_compare_excludes_bsg_from_params_and_includes_rpm_sections(self) -> None:
        ...
        with self.engine.begin() as conn:
            conn.execute(insert(recipe_params), [
                {"id": 1, "recipe_import_id": 1, "file_type": "PRM", "param_name": "parms/B1_Force_Seg_01", "param_value": "150"},
                {"id": 2, "recipe_import_id": 2, "file_type": "PRM", "param_name": "parms/B1_Force_Seg_01", "param_value": "150"},
                {"id": 3, "recipe_import_id": 1, "file_type": "BSG", "param_name": "bsg_dup_key", "param_value": "1"},
            ])
            conn.execute(insert(recipe_bsg), [
                {"id": 1, "recipe_import_id": 1, "ball_group": "BG1", "inspection_key": "IK1", "process_key": "PK1", "value": "V1"},
                {"id": 2, "recipe_import_id": 2, "ball_group": "BG1", "inspection_key": "IK1", "process_key": "PK1", "value": "V2"},
            ])
            conn.execute(insert(recipe_rpm_limits), [{"id": 1, "recipe_import_id": 1, "signal_name": "S1", "property_name": "P1", "rpm_group": "G1", "bond_type": "BT", "measurement_name": "M1", "limit_type": "L1", "statistic_type": "ST1", "parameter_set": "PS1", "lower_limit": 1.0, "upper_limit": 2.0, "active": True}])
            conn.execute(insert(recipe_rpm_reference), [{"id": 1, "recipe_import_id": 1, "signal_name": "S1", "property_name": "P1", "rpm_group": "G1", "bond_type": "BT", "measurement_name": "M1", "source": "SRC", "average": 1.0, "median": 1.0, "std_dev": 0.1, "median_abs_dev": 0.05, "minimum": 0.8, "maximum": 1.2, "sample_count": 12}])
        ...
        rpm_limits_payload = CompareRequest(import_ids=[1, 2], section="rpm_limits", show_all=True, page=1, page_size=10)
        rpm_reference_payload = CompareRequest(import_ids=[1, 2], section="rpm_reference", show_all=True, page=1, page_size=10)
        with self.engine.connect() as conn:
    ...
    def test_compare_response_pages_rows(self) -> None:
        ...
        with self.engine.begin() as conn:
            conn.execute(insert(recipe_params), [
                {"id": 1, "recipe_import_id": 1, "file_type": "PRM", "param_name": "parms/B1_Force_Seg_01", "param_value": "150"},
                {"id": 2, "recipe_import_id": 2, "file_type": "PRM", "param_name": "parms/B1_Force_Seg_01", "param_value": "151"},
                {"id": 3, "recipe_import_id": 1, "file_type": "PRM", "param_name": "parms/B2_Time_Seg_01", "param_value": "20"},
                {"id": 4, "recipe_import_id": 2, "file_type": "PRM", "param_name": "parms/B2_Time_Seg_01", "param_value": "22"},
            ])
    ...
    def test_compare_param_catalog_includes_facets_and_partial_presence(self) -> None:
        ...
        with self.engine.begin() as conn:
            conn.execute(insert(recipe_params), [
                ...
                {"id": 2, "recipe_import_id": 2, "file_type": "PRM", "param_name": "parms/B1_Force_Seg_01", "param_value": "151"},
                {"id": 3, "recipe_import_id": 1, "file_type": "PRM", "param_name": "parms/B2_Time_Seg_01", "param_value": "20"},
                {"id": 4, "recipe_import_id": 2, "file_type": "PHY", "param_name": "phy/Some_Param", "param_value": "1"},
            ])
        ...
        self.assertEqual({entry["value"] for entry in data["facets"]["file_types"]}, {"PRM", "PHY"})
    ...
    def test_compare_params_section_can_be_scoped_to_selected_keys(self) -> None:
        ...
        payload = CompareRequest(import_ids=[1, 2], section="params", show_all=True, selected_params=[CompareParamKey(file_type="PRM", param_name="parms/B2_Time_Seg_01")], page=1, page_size=10)
        with self.engine.connect() as conn:
            result = compare_recipe_params(payload, conn)
