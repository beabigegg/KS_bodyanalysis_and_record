...
#### Scenario: Delete single import with state clearing option
- **WHEN** a client sends `DELETE /api/imports/{import_id}?clear_state=true`
- **THEN** the system SHALL delete the import record and all related data, AND clear the corresponding entry in `FileStateStore` (using the record's `source_file`), allowing the scanner to re-import the file on next cycle

#### Scenario: Delete single import without state clearing
- **WHEN** a client sends `DELETE /api/imports/{import_id}` without `clear_state` parameter or with `clear_state=false`
- **THEN** the system SHALL delete only the database records, leaving the `FileStateStore` entry intact
...
- **THEN** the system SHALL show a confirmation dialog with a checkbox option "同時清除已處理標記（允許重新匯入）", and upon confirmation, call the delete API with the appropriate `clear_state` parameter and refresh the list
...
- **WHEN** a client sends `DELETE /api/imports/batch` with a JSON body containing `{"ids": [id1, id2, ...], "clear_state": false}`
...
#### Scenario: Batch delete with state clearing
- **WHEN** a client sends `DELETE /api/imports/batch` with `"clear_state": true`
- **THEN** the system SHALL delete all specified import records, clear `FileStateStore` entries for each record's `source_file`, and return the count of deleted records
...
- **THEN** the system SHALL show a confirmation dialog displaying the count of selected records and a checkbox option "同時清除已處理標記（允許重新匯入）", and upon confirmation, call the batch delete API with the appropriate `clear_state` parameter, clear selections, and refresh the list
