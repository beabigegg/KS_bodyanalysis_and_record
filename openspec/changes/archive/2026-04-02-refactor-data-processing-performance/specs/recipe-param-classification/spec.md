...
#### Scenario: ParamSemantics includes process_step field
- **WHEN** any PRM parameter is classified
- **THEN** the `ParamSemantics` dataclass SHALL include a `process_step` field (str | None) alongside existing stage, category, family, feature, instance, description, and tunable fields

#### Scenario: Lookup-covered parameter returns process_step
- **WHEN** `param_name = "parms/Bond1_Force_Seg_01"` and the process-step lookup contains this parameter
- **THEN** the classifier SHALL return the corresponding `process_step` value from the lookup

#### Scenario: Lookup-covered parameter corrects _unmapped stage
- **WHEN** a parameter was previously classified as `stage = "_unmapped"` but the process-step lookup provides `stage = "loop"`
- **THEN** the classifier SHALL return `stage = "loop"` instead of `"_unmapped"`
